...
struct AABB(size_t N){
    ...
    bool contains(Point!N point) {
        auto range = iota(0, N); // Generates a range from 0 to N-1
        return all!((i) => point.data[i] >= min.data[i] && point.data[i] <= max.data[i])(range);
    }

    bool intersects(AABB other) {
         auto range = iota(0, N); // Generates a range from 0 to N-1
        return all!((i) => max.data[i] >= other.min.data[i] && min.data[i] <= other.max.data[i])(range);
    }
}
