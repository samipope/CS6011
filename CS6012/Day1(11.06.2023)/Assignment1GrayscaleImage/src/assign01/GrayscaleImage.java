...
/**
    ...
    public double getPixel(int x, int y) {
        if (x < 0 || y < 0 || x >= imageData[0].length || y >= imageData.length) {
            throw new IllegalArgumentException("pixel coordinates (x, y) are out of bounds for the image");
        }
    ...
    public double averageBrightness() {
        ...
        for (var row = 0; row < imageData.length; row++) {
            for (var col = 0; col < imageData[0].length; col++) {
                double datapoint = imageData[row][col];
                totalDataSum = totalDataSum + datapoint;
            }
        }
        return totalDataSum / ((imageData.length) * imageData[0].length);
    }
    ...
    public GrayscaleImage mirrored() {
        int width = this.imageData.length;
        int height = this.imageData[0].length;
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height) {
        int originalWidth = this.imageData.length;
    ...
    public GrayscaleImage squarified() {
        ...
        int widthDiff = originalWidth - originalHeight;

        if (widthDiff == 0) {
            // the original image is already square, no changes needed
            return new GrayscaleImage(this.imageData);
        }

        int newSize = Math.min(originalWidth, originalHeight); // the size of the square image
        int removePixels = Math.abs(widthDiff) / 2; // calculate the number of pixels to remove

        double[][] squaredData = new double[newSize][newSize];

        if (widthDiff > 0) {
            // original width is greater, remove from left and right
            for (int y = 0; y < newSize; y++) {
                for (int x = 0; x < newSize; x++) {
                    squaredData[x][y] = this.imageData[removePixels + x][y];
                }
            }
        } else {
            // original height is greater, remove from top and bottom
            for (int y = 0; y < newSize; y++) {
                for (int x = 0; x < newSize; x++) {
                    squaredData[x][y] = this.imageData[x][removePixels + y];
                }
            }
        }
