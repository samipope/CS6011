...
struct Command {
    ...
    ~Command() {
        for (char* arg : argv) {
            free(arg); // Free the duplicated strings
        }
        if (inputFd != STDIN_FILENO) close(inputFd); // Close input file descriptor if not standard input
        if (outputFd != STDOUT_FILENO) close(outputFd); // Close output file descriptor if not standard output
    }
