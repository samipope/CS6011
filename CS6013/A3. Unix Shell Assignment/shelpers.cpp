//
// Created by Samantha Pope on 2/5/24.
//
...
bool splitOnSymbol( vector<string> & words, int i, char c )
{
    ...
    if( (pos = words[i].find(c)) != string::npos ){
        if( pos == 0 ){
        // Starts with symbol.
            words.insert( words.begin() + i + 1, words[i].substr(1, words[i].size() -
                                                                    1) );
...
vector<Command> getCommands( const vector<string> & tokens ){
    ...
    for (size_t cmdNumber = 0; cmdNumber < commands.size(); ++cmdNumber) {
        ...
        for (int j = first; j < last; ++j) {
            ...
            if (token == ">") {
                if (j + 1 < tokens.size()) {
                    int fd = open(tokens[j + 1].c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0666);
                    if (fd == -1) {
                        perror("open");
                        error = true;
                        break;
                    ...
                    std::cerr << "Error: No file specified for output redirection" << std::endl;
                    error = true;
                    break;
                }
            } else if (token == "<") {
                if (j + 1 < tokens.size()) {
                    ...
                    std::cerr << "Error: No file specified for input redirection" << std::endl;
                    error = true;
                    break;
                }
            } else if (j == first) {
        ...
        if (!error) {
            command.argv.push_back(nullptr); // Null-terminate argv
        } else {
            // In case of an error, clean up any allocated resources
